...
function createTerrain(layout) {
  const terrain = new THREE.Mesh(
    new THREE.PlaneGeometry(TERRAIN_WIDTH, TERRAIN_DEPTH, 88, 60),
...
function createPathRibbon(layout) {
  const points = layout.descentPath.points;
  const vertices = [];
  const indices = [];
  for (let index = 0; index < points.length; index += 1) {
    ...
    const nx = -dz / length;
    const nz = dx / length;
    const y = point.y + 0.028;
    vertices.push(
      point.x + nx * point.width * 0.5, y, point.z + nz * point.width * 0.5,
      point.x - nx * point.width * 0.5, y, point.z - nz * point.width * 0.5,
    );
    if (index > 0) {
      const base = index * 2;
...
export function createRebuildBlockout({ scene, groundSurfaces, registerWalkableSurface, addCollider, registerNpc = null, layout }) {
  ...
  for (const segment of layout.ridge.segments) {
    nearRidge.add(createBackdropHill({
      ...segment,
      y: segment.y,
      color: 0x66745e,
    }));
  ...
  const backdropHills = [
    [-48, 28, 13, 7, 11], [-50, 4, 15, 8, 12], [-44, -48, 12, 6, 10],
    [55, 29, 15, 7, 12], [57, 1, 17, 9, 14], [52, -31, 14, 7, 11],
    [-78, -57, 16, 7, 13], [-121, -58, 17, 8, 14], [-35, 55, 15, 7, 12], [24, 55, 15, 7, 12],
  ];
