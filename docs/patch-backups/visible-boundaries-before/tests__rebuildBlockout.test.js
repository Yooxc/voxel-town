...
test("keeps the arrival hill above the market and creates a downhill transition", () => {
  ...
  const hillHeight = getRebuildBlockoutHeight(0, 27, layout.origin);
  const middleHeight = getRebuildBlockoutHeight(0, 8, layout.origin);
  const marketHeight = getRebuildBlockoutHeight(0, -12, layout.origin);

  assert.equal(hillHeight, 6);
...
test("places the rendered terrain under the door, spawn, and market elevations", () => {
  ...
  assert.equal(blockout.nearRidge.name, "EXCIT_REBUILD_NEAR_RIDGE");
  assert.equal(blockout.replaceableBackdrop.name, "EXCIT_REBUILD_REPLACEABLE_BACKDROP");
});
