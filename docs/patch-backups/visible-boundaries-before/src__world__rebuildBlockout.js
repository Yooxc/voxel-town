...
export function createRebuildBlockout({ scene, groundSurfaces, registerWalkableSurface, addCollider, registerNpc = null, layout }) {
  ...
  nearRidge.add(createPolygonBand(layout.ridge.outline, ridgeCrest, layout.origin.y - 0.2, layout.origin.y + 13, ridgeMaterial));
  ...
  const backdropHills = [
    [-52, 33, 27, 11, 24], [-92, 24, 32, 13, 24], [-135, 15, 27, 10, 24],
    [-182, -40, 18, 14, 38], [-139, -88, 38, 12, 13], [-88, -88, 28, 10, 13],
  ...
  for (const anchor of Object.values(layout.expansionAnchors)) {
    replaceableBackdrop.add(createBackdropHill({
      x: anchor.x,
      y: anchor.y,
      z: anchor.z,
  ...
  // The far terrain is scenery, not an extension of the playable map bounds.
  const boundsSurface = new THREE.Mesh(new THREE.PlaneGeometry(232, 115), new THREE.MeshBasicMaterial({ visible: false }));
  boundsSurface.rotation.x = -Math.PI / 2;
  boundsSurface.position.set(layout.origin.x - 63, layout.origin.y, layout.origin.z - 21);
  root.add(boundsSurface);
  registerWalkableSurface(layout.mapId, boundsSurface, 0);
